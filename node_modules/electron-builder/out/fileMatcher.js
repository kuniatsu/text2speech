...
function createFileMatcher(appDir, resourcesPath, macroExpander, platformSpecificBuildOptions, packager) {
    var _matcher$patterns;

    const buildResourceDir = _path.resolve(packager.info.projectDir, packager.buildResourcesDir);
    const patterns = packager.info.isPrepackedAppAsar ? null : getFileMatchers(packager.info.config, "files", appDir, _path.join(resourcesPath, "app"), false, macroExpander, platformSpecificBuildOptions);
    ...
    (_matcher$patterns = matcher.patterns).unshift.apply(_matcher$patterns, customFirstPatterns);
    ...
    matcher.addPattern("!**/{.git,.hg,.svn,CVS,RCS,SCCS," + "__pycache__,.DS_Store,thumbs.db,.gitignore,.gitattributes," + ".editorconfig,.flowconfig,.jshintrc,.eslintrc," + ".yarn-integrity,.yarn-metadata.json,yarn-error.log,yarn.lock,npm-debug.log," + ".idea,.vs," + "appveyor.yml,.travis.yml,circle.yml," + ".nyc_output}");
    return matcher;
}
/** @internal */
